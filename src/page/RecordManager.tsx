...
import { getRecords, addRecord, removeRecord } from '../util/grpc'

export default function RecordManager(props: RouteComponentProps<{ name: string }>) {
  ...
  useEffect(() => {
    (async () => {
      const res = await getRecords(token, name).catch((e) => console.error(e));
      if (!res) {
        console.log(res);
        return;
      }
  ...
  async function handleAdd() {
    const res = await addRecord(token, rname, name, rtype, rttl, rcontent).catch((e) => console.error(e));
    if (!res) {
      console.log(res);
      return;
    }
    const nres = await getRecords(token, name).catch((e) => console.error(e));
    if (!nres) {
      return;
    }
    setRecords(nres.records.map(r => ({
      name: r.getName(),
      type: r.getType(),
      ttl: r.getTtl(),
      content: r.getContent(),
  ...
  async function handleRemove(rn: string, rt: RRType, rc: string) {
    const res = await removeRecord(token, rn, name, rt, rc).catch((e) => console.error(e));
    if (!res) {
      return;
    }
